"""
TRAIN ALL V4 — los 5 modelos con SPLITS UNIFICADOS (test = 2025 en los 3 exps).
Versión final para el paper.
"""
import os
...
from common import (TICKERS, CLASES, metricas_full, metricas_global_por_ticker,
                    guardar_json, imprimir_metricas, OUT_DIR, cargar_raw, get_feature_cols)
from common_v4 import cargar_dataset_v4, cargar_global_v4, SPLITS_V4, EXPERIMENTOS
...
RESULTS_CSV = OUT_V4 / "resultados_v4.csv"
...
class CNNPuro(nn.Module):
    ...
    def forward(self, x):
        x = x.transpose(1,2); x = self.cnn(x)
...
# ════════════════════════════════════════════════════════════════════════════
# DATOS SECUENCIALES (con SPLITS_V4)
# ════════════════════════════════════════════════════════════════════════════
...
def run_xgb(exp_id, resultados):
    ...
    def buscar(X_tr, y_tr, X_va, y_va, n_trials=20):
        sw = compute_sample_weight("balanced", y=y_tr)
        def obj(t):
            p = {"n_estimators": t.suggest_int("n_estimators", 200, 600),
    ...
    for tk in TICKERS:
        d = cargar_dataset_v4(tk, exp_id)
        best = buscar(d["X_tr"], d["y_tr"], d["X_va"], d["y_va"], n_trials=20)
    ...
    g = cargar_global_v4(exp_id)
    best = buscar(g["X_tr"], g["y_tr"], g["X_va"], g["y_va"], n_trials=40)
...
def main():
    print("="*70)
    print("  TRAIN ALL V4 — splits unificados (test=2025 en los 3 exps)")
    print("="*70)
