...
Candle patterns & 7 & body/range, upper/lower shadow, gap, high/low ratio \\
...
Since all three splits share it, the comparison rests on one out-of-sample period.
Averaged over the seven tickers, 2025 returned $+24.2\,\%$ against a 2014--2024
median of $+46.3\,\%$ (9th of twelve years), with realized volatility $39.1\,\%$
(3rd highest), mean maximum drawdown $-33.0\,\%$ (median $-21.4\,\%$) and mean VIX
19.0; label balance was 29.6/42.2/28.3 against 30.0/40.0/30.0 over the full sample.
All four market statistics fall inside the 2014--2024 range, so no model
extrapolates, but the year is below-median on return and above-median on
volatility and drawdown --- not the easy bull market that would flatter a
long-biased classifier. That makes 2025 a defensible single choice, not a
substitute for multiple test years (Section~\ref{sec:limitations}).
...
We report 120 controlled runs (5 models $\times$ 3 splits $\times$ (1 global + 7 per-ticker)). For the three sequence models (LSTM, CNN, CNN-LSTM) two look-back windows were trained (20 and 60 days) and this section reports, for each run, the better of the two \emph{as measured on the test set}. That choice is deliberately generous: it gives the deep models an oracle advantage that LR and XGBoost do not get, so the gaps reported below are a lower bound on LR's margin. Section~\ref{sec:robust} repeats the comparison under a protocol in which no test-set information is used for any selection.
...
Table~\ref{tab:tsla_drill} reports the seven per-ticker XGBoost models of Exp B, the setting with the highest global F1. TSLA is the outlier (F1 0.44, Sharpe +2.06, +201\,\% cumulative return) and GOOGL the worst, negative on both. The best-to-worst spread across tickers (0.44 vs.\ 0.34 in F1, +2.06 vs.\ $-0.88$ in Sharpe) is far wider than the spread across model classes in Table~\ref{tab:f1_global}.

\begin{table}[t]
\centering
\caption{Per-ticker XGBoost models of Exp B (test = 2025), ordered by F1-macro; META (0.347) and AAPL (0.336) are omitted for space.}
...
Two asymmetries in Section~\ref{sec:results} could by themselves explain the
ranking: XGBoost was tuned by an automated Optuna search while the deep models got
only a small hand-built grid over \texttt{hidden}, \texttt{layers} and
\texttt{dropout} --- one that did not contain the learning rate or the weight decay
at all --- and their look-back was picked on the test set. We re-ran the whole
comparison under a protocol that removes both.
...
Every model is now tuned by the same Optuna TPE sampler on the same decision year
(Table~\ref{tab:robust}). The deep models search 17--20 hyper-parameters ---
learning rate, weight decay, cross-entropy versus focal loss, label smoothing, the
temporal pooling operator, the scaler and the look-back itself --- against the 9 of
XGBoost and the 5 of LR; of the 80 trials each, 54 (LSTM), 53 (CNN 1D) and 74
(CNN-LSTM) ran to completion and the rest were pruned by a median pruner. The
search is cheap: one full deep training run takes
7--12\,s on a single consumer GPU, so the binding constraint here is statistical
power, not compute.
...
checkpoint was effectively the first or second. Three diagnostics bound what tuning
can achieve here. Validation loss never falls appreciably below $\ln 3 = 1.0986$ in
any configuration tried, and shrinking the LSTM from 592\,k to 10\,k parameters
changes nothing, so the limit is neither capacity nor regularization. Retraining on
train\,$+$\,validation, which only LR and XGBoost did, makes all three deep models
\emph{worse} ($-0.024$ to $-0.047$). And an implementation change that leaves the
mathematics untouched moved the LSTM's test F1 by $0.023$ through floating-point
reduction order alone --- hence the means over seeds.

\section{Discussion}
\label{sec:discussion}

\paragraph{Why does LR win on F1?}
With approximately 1500 training samples per ticker (10\,500 globally) and 61 inputs, deep models with $10^5$--$10^6$ parameters are firmly in an overfitting regime. The technical indicators already capture most of the linear predictive content of price action; the elasticnet penalty selects an effective subset of them automatically and stabilizes the remaining coefficients. XGBoost shows similar resilience because tree pruning and the $\ell_2$ leaf penalty play the same role.

This matches a broader finding: on tabular data with engineered features, tree-based and linear models often match or beat deep networks, whose inductive biases target raw spatial or sequential signals rather than engineered columns~\cite{grinsztajn2022,shwartzziv2022}. The asset-pricing literature is instructive about \emph{when} that ordering flips. Gu et al.~\cite{gukellyxiu2020} do rank neural networks first, but on roughly 30\,000 stocks over 60 years --- orders of magnitude more label-bearing rows than the 10\,500 here --- and for a continuous excess-return target rather than a three-way daily sign; Fischer and Krauss~\cite{fischer2018} likewise report LSTM gains on the full S\&P 500 rather than seven tickers, while Patel et al.~\cite{patel2015} reach our conclusion at a comparable sample size. Our result is therefore not evidence against deep learning for financial time series: it locates the regime where it does not pay --- short daily histories, a near-noise one-day label, and features that are already multi-day aggregates --- and Section~\ref{sec:robust} shows a serious search does not rescue it.
...
Pooling all tickers into one training set helps XGBoost in all three experiments and the deep models in seven of nine (Tables~\ref{tab:f1_global} and~\ref{tab:f1_pt}), which is expected when it multiplies the sample by seven. For LR it is a wash --- per-ticker is marginally better in Exps A and B, clearly worse in Exp C. Pooling thus buys most for the sample-hungry models and nothing for the one that already regularizes hardest.

\paragraph{When should one use the deep models?}
Under the protocol of Section~\ref{sec:results} the LSTM and CNN-LSTM at least contain drawdown better than LR ($-0.41$ and $-0.40$ against $-0.47$), but that advantage does not survive Section~\ref{sec:robust}, where their drawdowns are the worst of the five ($-0.68$ and $-0.62$). They become attractive when the application needs temporal dependencies beyond the look-back already baked into the features, and when there is enough history to justify the parameter count. Here neither condition is met.
...
The main limitation is that all three splits test on 2025. Section~\ref{sec:data} argues the year is not anomalous and Section~\ref{sec:robust} attaches bootstrap intervals, but one period cannot fully separate a model effect from a year-specific regime; rolling the protocol over several consecutive test years remains the most valuable extension, and the run-to-run sensitivity of Section~\ref{sec:robust} suggests it would widen the intervals. Two caveats follow. The search is symmetric in \emph{procedure} but not in \emph{dimension} --- 80 trials over 17--20 hyper-parameters explore a deep space far more sparsely than 150 over 9 explore XGBoost's --- so we claim only that a realistic search does not overturn the ranking. And the conclusion is scoped to this representation: multi-day aggregates stacked over a short window are close to the worst case for a sequence model.

\section{Conclusions}
\label{sec:conclusions}

A multinomial logistic regression with elasticnet regularization, trained on 61 indicator features from free Yahoo Finance data, beats three deep architectures and a tuned XGBoost on average F1-macro across three temporal splits and matches or exceeds them economically --- and its margin survives a symmetric hyper-parameter search, a selection protocol that touches no test data, and bootstrap confidence intervals. Statistical feature pre-selection does not help: keeping all features and relying on the model's own regularization wins in 11 of 15 cases. Ticker volatility, not model class, explains the bulk of the cross-asset Sharpe variation. For daily large-cap data with no alternative datasets, a well-regularized linear classifier is a competitive and cheap default; deep models become attractive only with longer histories or richer inputs.
...
Shwartz-Ziv, R., Armon, A.: Tabular data: Deep learning is not all you need. Information Fusion \textbf{81}, 84--90 (2022)
