"""
================================================================================
SCRIPT 06 — ENTRENAMIENTO: LSTM (PyTorch + CUDA)
================================================================================
Arquitectura LSTM para clasificación de señales de trading.
Soporta GPU (CUDA) y CPU automáticamente.
Estrategias: por ticker y modelo global.
Lookbacks: 20 y 60 días.
...
Salida: tesis_ml_stocks/04_models/lstm/
    por_ticker/lookback_[20|60]/experimento_[A|B|C]/
        {TICKER}_modelo.pt
        {TICKER}_checkpoint.pt
        {TICKER}_metricas.json
    global/lookback_[20|60]/experimento_[A|B|C]/
        modelo_global_checkpoint.pt
        metricas_global.json
    resultados_lstm.csv
...
DATA_BASE  = Path("tesis_ml_stocks/03_model_datasets/lstm")
RAW_DIR    = Path("tesis_ml_stocks/01_raw_datasets")
OUTPUT_DIR = Path("tesis_ml_stocks/04_models/lstm")
...
class LSTMClasificador(nn.Module):
    """
    Arquitectura LSTM para clasificación multiclase de señales de trading.
    Estructura:
        Input → LSTM (n_layers, hidden_size, dropout) → LayerNorm → Dropout → Linear
    El estado oculto del último paso de tiempo se usa para la clasificación.
    """
    def __init__(self, input_size: int, hidden_size: int = HIDDEN_SIZE,
        ...
        self.fc      = nn.Linear(hidden_size, n_clases)
...
def calcular_class_weights(y_train):
    pesos = compute_class_weight("balanced", classes=np.array(CLASES), y=y_train)
...
def entrenar_ciclo(modelo, loader_tr, loader_va, class_weights,
                   epochs=EPOCHS, patience=PATIENCE, label=""):
    """
    Entrenamiento con early stopping basado en val_loss.
    Guarda los pesos del mejor modelo (menor val_loss) en memoria.
    """
    ...
    for lb in LOOKBACKS:
        for exp_id, exp_desc in EXPERIMENTOS.items():
            ...
            for ticker in TICKERS:
                ...
                loader_tr  = crear_dataloader(X_tr, y_tr, shuffle=True)
                loader_va  = crear_dataloader(X_va, y_va, shuffle=False)

                modelo = LSTMClasificador(input_size=input_size).to(DEVICE)
                modelo, historial = entrenar_ciclo(
                    modelo, loader_tr, loader_va, cw, label=ticker)
                ...
                torch.save(modelo.state_dict(), dir_out / f"{ticker}_modelo.pt")
                torch.save({
                ...
                meta = {
                    ...
                    "lookback": lb, "tipo": "por_ticker", "modelo": "LSTM",
                    "arquitectura": {
                    ...
                    "entrenamiento": {
                        ...
                        "mejor_f1_val":   max(h["f1_va"]   for h in historial),
                    },
...
def pipeline():
    print("="*65)
    print("  SCRIPT 06 — LSTM (PyTorch)")
    print(f"  Dispositivo: {DEVICE}")
    print("="*65)

    res = entrenar_por_ticker() + entrenar_global()
    df  = pd.DataFrame(res)
    df.to_csv(OUTPUT_DIR / "resultados_lstm.csv", index=False)
